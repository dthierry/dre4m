...
function writeRes(m::JuMP.Model, mS::modSets, mD::modData, pr::prJrnl
  ...
  XLSX.openxlsx(fname*"_stats.xlsx", mode="w") do xf
    ...
    sheet["B3"] = value(npv)
    sheet["B4"] = sum(value(wRet[i, j]) for i in 0:I-1 for j in 0:N[i]-1) + 
    sum(value(zRet[i, k, j]) for i in 0:I-1 for k in 0:Kz[i]-1 for j in 0:N[i]-1) + 
    sum(value(xRet[i, k, j]) for i in 0:I-1 for k in 0:Kx[i]-1 for j in 0:T-1)
  ...
  XLSX.openxlsx(fname*"_zx_cap.xlsx", mode="w") do xf
    ...
    for i in 0:I-1
      ...
      for k in 0:Kz[i]-1
        ...
        sheet["B$(row)"] = sum(value(zTrans[t, i, k, j]) 
